...
```
기존 코드에서 chatroom에 name을 추가했더니 기존의 있던 채팅방들 때문에 migrate오류가 발생하여 db와 그전에 했던 migrations를 삭제하였습니다.
...
## 7. 개발하면서 느낀점
- 이번 프로젝트를 진행하면서 Django Rest Framework (DRF)를 활용했습니다. DRF를 처음 사용해보면서 많은 도전이 있었으며, 이를 통해 새로운 것을 배울 수 있는 기회가 되었습니다. 특히, 기능 구현 과정에서 다양한 문제점을 마주하게 되었고, 이러한 문제를 해결하기 위해 다른 개발자들의 코드를 참조하거나 검색을 통해 필요한 정보를 찾는 등의 노력을 기울였습니다.
그러나, 오류 발생 원인과 해결 방법을 찾는 과정에서 어려움을 겪었습니다. 오류의 원인을 파악하고, 이를 해결하는 데 상당한 시간이 소요되었습니다. 이 과정에서 제가 DRF에 대해 아직 완벽히 이해하지 못하고 있다는 것을 깨달았습니다.
또한, DRF의 복잡성에 대한 예상을 넘어서는 경험을 했습니다. 이로 인해 처음 기획했던 일부 기능을 구현하지 못한 점은 아쉬움으로 남습니다. 그러나 이 경험은 저에게 어떤 부분을 더 학습하고 개선해야 하는지를 깨닫게 해주었습니다.
